...
public class AsignaturaController implements Serializable {
	...
	@PostConstruct
	public void init() {
		evaluaciones = evaluacionService.obtenerEvaluacionesPorAsignatura(asignatura);
		gruposDeEvaluacion = evaluacionService.obtenerGruposDeEvaluacionActivos(periodoLectivo);
	}

	public Boolean getRenderizarInsumos() {
		return periodoLectivo.getInsumos() != null && periodoLectivo.getInsumos() > 2;
	}
        

	public Boolean getRenderizarParciales() {
		return periodoLectivo.getParciales() != null && periodoLectivo.getParciales() > 2;
	}
	...
	public void guardarTodo() {
		try {
			for (EvaluacionAsignatura evaluacion : evaluaciones) {
				evaluacionService.evaluarEvaluacionAsignatura(evaluacion);
			}
			facesContext.addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Éxito", "Evaluaciones registradas con éxito"));
	...
	public void setEvaluaciones(List<EvaluacionAsignatura> detallesInformesAnuales) {
